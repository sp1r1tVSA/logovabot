...
3. **ЛЕВАЯ ПОЛОВИНА (LEFT SIDE):**
   ...
   - Столбец `Г` / `G` (Голы) идет ПЕРВЫМ из двух правых цифр (дальше от центра).
   - Столбец `А` / `A` (Ассисты) идет ВТОРЫМ (ближе всего к центру экрана).
   - Занеси ИМЯ ИГРОКА в `left_goals`, если число в столбце `Г`/`G` > 0 (столько раз, чему равно число).
   - Занеси ИМЯ ИГРОКА в `left_assists`, если число в столбце `А`/`A` > 0 (столько раз, чему равно число).

4. **ПРАВАЯ ПОЛОВИНА (RIGHT SIDE) — ВНИМАНИЕ, ЗЕРКАЛЬНЫЙ ПОРЯДОК:**
   - Порядок столбцов: `А/A` | `Г/G` | `ИС/PS` | `ОБЩ/OVR` | `ИГРОКИ/PLAYERS` | `ПОЗ/POS`
   - Столбец `А` / `A` (Ассисты) идет ПЕРВЫМ (ближе всего к центру экрана).
   ...
   - ПРИМЕР:
     ...
     -> 1 (первое число) = АССИСТЫ (A = 1) -> занеси "Leweling" в `right_assists`!
     -> 0 (второе число) = ГОЛЫ (G = 0)
   - СТРОГО: первое число слева в правой таблице — это АССИСТЫ (А/A), второе — ГОЛЫ (Г/G). НЕ ПЕРЕПУТАЙ!
   - Занеси ИМЯ ИГРОКА в `right_goals`, если число в столбце `Г`/`G` > 0 (столько раз, чему равно число).
   - Занеси ИМЯ ИГРОКА в `right_assists`, если число в столбце `А`/`A` > 0 (столько раз, чему равно число).
