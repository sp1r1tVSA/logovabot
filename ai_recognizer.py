...
- Из этого типа берутся ТОЛЬКО: `left_score`, `right_score` и имена забивших в `left_goals` / `right_goals`.
...
5. **ОБЯЗАТЕЛЬНАЯ КРОСС-ПРОВЕРКА:**
   ...
   - Если количество забитых голов не совпадает со счётом на табло, ты перепутал столбцы `Г` и `А` в одной из таблиц. Вспомни: в правой таблице АССИСТЫ (А) идут перед ГОЛАМИ (Г)!
