...
4. **ПРАВАЯ ПОЛОВИНА (RIGHT SIDE) — ВНИМАНИЕ, ЗЕРКАЛЬНЫЙ ПОРЯДОК:**
   ...
   - ПРИМЕРЫ:
     ...
     -> 0 (второе число, колонка Г) = ГОЛЫ (G = 0)
     Если в строке Gittens стоит: 0  1  114 Gittens ЛП
     -> 0 (первое число, колонка А) = АССИСТЫ (A = 0)
     -> 1 (второе число, колонка Г) = ГОЛЫ (G = 1) -> занеси "Gittens" в `right_goals`!
     Если в строке Raspadori стоит: 0  1  111 Raspadori ЦАП
     -> 0 (первое число, колонка А) = АССИСТЫ (A = 0)
