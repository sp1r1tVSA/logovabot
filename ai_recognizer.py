...
3. **ЛЕВАЯ ПОЛОВИНА (LEFT SIDE):**
   ...
   - Занеси ИМЯ ИГРОКА в `left_assists`, если число в столбце `А`/`A` > 0 (СТРОГО столько раз, чему равно число).
...
4. **ПРАВАЯ ПОЛОВИНА (RIGHT SIDE) — ВНИМАНИЕ, ЗЕРКАЛЬНЫЙ ПОРЯДОК:**
   ...
   - ПРИМЕРЫ:
     Если в строке Renato Sanches стоит: 0  2  103 Renato Sanches ЦП
     -> 0 (первое число, колонка А) = АССИСТЫ (A = 0) -> в `right_assists` НЕ ДОБАВЛЯТЬ!
     -> 2 (второе число, колонка Г) = ГОЛЫ (G = 2) -> занеси "Renato Sanches" ДВАЖДЫ в `right_goals`!
     Если в строке El Shaarawy стоит: 1  0  104 El Shaarawy ЛВ
     -> 1 (первое число, колонка А) = АССИСТЫ (A = 1) -> занеси "El Shaarawy" в `right_assists`!
     -> 0 (второе число, колонка Г) = ГОЛЫ (G = 0) -> в `right_goals` НЕ ДОБАВЛЯТЬ!
     Если в строке Ito стоит: 0  1  101 Ito ПВ
     -> 0 (первое число, колонка А) = АССИСТЫ (A = 0) -> в `right_assists` НЕ ДОБАВЛЯТЬ!
     -> 1 (второе число, колонка Г) = ГОЛЫ (G = 1) -> занеси "Ito" в `right_goals`!
     Если в строке Carlos Forbs стоит: 1  0  70 Carlos Forbs ЛВ
     -> 1 (первое число, колонка А) = АССИСТЫ (A = 1) -> занеси "Carlos Forbs" в `right_assists`!
     -> 0 (второе число, колонка Г) = ГОЛЫ (G = 0) -> в `right_goals` НЕ ДОБАВЛЯТЬ!
