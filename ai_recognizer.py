...
def recognize_match_screenshots_bytes(images_bytes_list: list[bytes], mime_type: str = "image/jpeg") -> dict | None:
    ...
    prompt_text = (
        ...
        "=== КРИТИЧЕСКОЕ ПРАВИЛО ДЛЯ КОЛОНОК 'Г' И 'А' (EA FC / FIFA) ===\n"
        "Внимательно смотри на буквы в заголовках колонок над цифрами:\n"
        "1. ДЛЯ ЛЕВОЙ КОМАНДЫ заголовки идут слева направо: [ Г | А ] (Г = Голы, А = Ассисты).\n"
        "   - Левая цифра = Голы (Г), Правая цифра = Ассисты (А).\n"
        "   - Пример: 'Quintero | 1 | 1' -> 1 Гол, 1 Ассист.\n"
        "   - Пример: 'Martínez | 1 | 0' -> 1 Гол, 0 Ассистов.\n\n"
        "2. ДЛЯ ПРАВОЙ КОМАНДЫ заголовки ЗЕРКАЛЬНЫЕ: [ А | Г ] (А = Ассисты, Г = Голы)!\n"
        "   - Первая (ЛЕВАЯ) цифра под колонкой 'А' = АССИСТЫ (Assists)!\n"
        "   - Вторая (ПРАВАЯ) цифра под колонкой 'Г' = ГОЛЫ (Goals)!\n"
        "   - ПРИМЕР: строка '0 | 1 | Vargas' под 'А | Г' означает: 0 Ассистов (А=0), 1 Гол (Г=1). У Vargas НЕТ ассиста! Не добавляй его в side2_assists!\n"
        "   - ПРИМЕР: строка '0 | 1 | Lukébakio' под 'А | Г' означает: 0 Ассистов (А=0), 1 Гол (Г=1). У Lukébakio НЕТ ассиста! Не добавляй его в side2_assists!\n"
        "   - ПРИМЕР: строка '2 | 0 | Elyounoussi' под 'А | Г' означает: 2 Ассиста (А=2), 0 Голов (Г=0). Elyounoussi сделал 2 АССИСТА! Запиши Elyounoussi ДВАЖДЫ в side2_assists: [\"Elyounoussi\", \"Elyounoussi\"]!\n\n"
        "ПРАВИЛО ПРОВЕРКИ:\n"
        "- Игрок попадает в список голов (goals) ТОЛЬКО если у него цифра под колонкой 'Г' больше 0.\n"
        "- Игрок попадает в список ассистов (assists) ТОЛЬКО если у него цифра под колонкой 'А' больше 0.\n"
        "- Количество упоминаний имени игрока в списке ДОЛЖНО РАВНЯТЬСЯ цифре в соответствующей колонке (например, цифра 2 под А = 2 раза записать имя в список ассистов).\n\n"
