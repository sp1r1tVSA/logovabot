...
### ЭТАП 1: АНАЛИЗ СЧЁТА И ГЕОМЕТРИИ ЭКРАНА
...
2. **СХЕМА ЗАГОЛОВКОВ И СТОЛБЦОВ СТАТИСТИКИ ИГРОКОВ:**

ЛЕВАЯ СТОРОНА:                          [ЦЕНТР]         ПРАВАЯ СТОРОНА:
[POS] [PLAYERS] [OVR] [PS] [ G ] [ A ]    ||    [ A ] [ G ] [PS] [OVR] [PLAYERS] [POS]

3. **ЛЕВАЯ ПОЛОВИНА (LEFT SIDE):**
   - Заголовки над числами: `G` (Голы), затем `A` (Ассисты).
   - Первая цифра после OVR = ГОЛЫ игрока (G).
   - Вторая цифра (ближняя к центральной линии) = АССИСТЫ игрока (A).
   - Занеси ИМЯ ИГРОКА из `PLAYERS` в `left_goals` столько раз, чему равно значение G.
   - Занеси ИМЯ ИГРОКА из `PLAYERS` в `left_assists` столько раз, чему равно значение A.

4. **ПРАВАЯ ПОЛОВИНА (RIGHT SIDE) — ВНИМАНИЕ, ПОРЯДОК ЗЕРКАЛЬНЫЙ!**
   - Заголовки над числами: `A` (Ассисты), затем `G` (Голы).
   - Первая цифра (ближайшая к центральной линии) = АССИСТЫ игрока (A).
   - Вторая цифра (перед OVR) = ГОЛЫ игрока (G).
   - Занеси ИМЯ ИГРОКА из `PLAYERS` в `right_goals` столько раз, чему равно значение G.
   - Занеси ИМЯ ИГРОКА из `PLAYERS` в `right_assists` столько раз, чему равно значение A.

5. **ОБЯЗАТЕЛЬНАЯ КРОСС-ПРОВЕРКА СЧЁТА:**
   - Общее количество элементов в `left_goals` ОБЯЗАНО совпадать с `left_score`!
   - Общее количество элементов в `right_goals` ОБЯЗАНО совпадать с `right_score`!
   - Если количество забитых голов у игроков не совпадает со счётом на табло, перепроверь значения G у всех игроков!
