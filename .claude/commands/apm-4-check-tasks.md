...
1. Determine registration state:
   ...
   - If not registered, `$ARGUMENTS` is required. If no argument provided, inform User that an agent-id is required.

2. Resolve agent-id (unregistered Workers only): resolve `$ARGUMENTS` against `.apm/bus/` directory names per `.claude/skills/apm-communication/SKILL.md` §4.2 Agent ID Resolution. Initialize per `.claude/commands/apm-3-initiate-worker.md` §2 Initiation.
...
4. Cross-validate `agent` field in YAML frontmatter against registered identity. Mismatch flags a routing error - decline and direct User to the correct Worker. Process the Task per `.claude/apm-guides/task-execution.md` §3 Task Execution Procedure.
