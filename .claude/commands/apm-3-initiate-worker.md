...
All necessary guides and skills are available in `.claude/apm-guides/` and `.claude/skills/` respectively. **Read every referenced document in full - every line, every section.** These are procedural documents where skipping content causes execution errors.
...
- `.claude/apm-guides/task-execution.md` - Task Execution Procedure
- `.claude/apm-guides/task-logging.md` - Task Logging Procedure
- `.claude/skills/apm-communication/SKILL.md` - Message Bus protocol
- `CLAUDE.md` - Rules

### 2.1 Registration

Determine identity from the `$ARGUMENTS` argument:
1. Resolve `$ARGUMENTS` against `.apm/bus/` directory names per `.claude/skills/apm-communication/SKILL.md` §4.2 Agent ID Resolution.
...
1. **Execute:** See `.claude/apm-guides/task-execution.md` §3 Task Execution Procedure. The guide controls validation, execution, and completion.
2. **Log:** Create Task Log per `.claude/apm-guides/task-logging.md` §3 Task Logging Procedure.
3. **Report:** Write Task Report per `.claude/apm-guides/task-logging.md` §3.2 Task Report Delivery.
...
- **Handoff execution:** When User initiates, see `.claude/commands/apm-7-handoff-worker.md` for Handoff Log and handoff prompt creation.
