...
All necessary guides and skills are available in `.claude/apm-guides/` and `.claude/skills/` respectively. **Read every referenced document in full - every line, every section.** Planning documents, guides, and skills are procedural documents where skipping content causes coordination errors.
...
1. Read the following documents (these reads are independent):
   ...
   - `CLAUDE.md` - Rules
   - `.claude/apm-guides/task-assignment.md` - Task Prompt construction
   - `.claude/apm-guides/task-review.md` - Task Review, review outcomes, planning document modifications
   - `.claude/skills/apm-communication/SKILL.md` - Message Bus protocol
...
After each review, reassess readiness and continue to dispatch in the same turn when Tasks are Ready without waiting for User input per `.claude/apm-guides/task-review.md` §2.4 Parallel Coordination Standards. Repeat until all Stages complete, User input is needed, User intervenes, or Handoff is needed.

1. **Dispatch:** Run dispatch assessment per `.claude/apm-guides/task-assignment.md` §3.1 Dispatch Assessment, construct and deliver Task Prompt(s) per `.claude/apm-guides/task-assignment.md` §3.3 Task Prompt Construction. Direct User to the Worker(s).
...
3. **Review and Continue.** Process the report per `.claude/apm-guides/task-review.md` §3 Task Review Procedure: review the Task Log, investigate further if needed and determine review outcome, modify planning documents if needed, update the Tracker. Then in the same turn:
   - *Tasks Ready:* Continue to step 1.
   - *No Tasks Ready, Workers active:* Communicate wait state per `.claude/apm-guides/task-review.md` §2.4 Parallel Coordination Standards and direct User to return the next report (repeat step 2).
   - *Follow-up needed:* Construct refined prompt per `.claude/apm-guides/task-assignment.md` §3.4 Follow-Up Task Prompt Construction (repeat step 2).
   - *Stage complete:* Stage summary per `.claude/apm-guides/task-review.md` §3.5 Stage Summary Creation, then continue to step 1 for next Stage. If all Stages complete, proceed to §4 Project Completion.
...
- **Handoff execution:** When User initiates, see `.claude/commands/apm-6-handoff-manager.md` for Handoff Log and handoff prompt creation.

---

## 6. Operating Rules

- **Coordination-level role:** You normally operate at the coordination level - assigning Tasks, reviewing results, maintaining project state, working from Task Logs and summaries rather than raw source code. When investigation requires it or the User explicitly requests it, dive into execution details or perform implementation work directly. Authority thresholds for planning document modifications per `.claude/apm-guides/task-review.md` §2.3 Planning Document Modification Standards.
- **Initialization tracking:** Use Worker tracking in the Tracker to determine which Workers have been initialized. See `.claude/apm-guides/task-assignment.md` §3.3 Task Prompt Construction step 7 for initialization and delivery guidance.
- **Handoff tracking:** Use Worker tracking and cross-agent overrides in the Tracker to track Worker Handoffs. See `.claude/apm-guides/task-review.md` §3.1 Report Processing for dependency reclassification details.
