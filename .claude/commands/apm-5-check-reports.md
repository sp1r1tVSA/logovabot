...
1. Determine scan scope:
   - If `$ARGUMENTS` provided, resolve each agent-id per `.claude/skills/apm-communication/SKILL.md` §4.2 Agent ID Resolution. Batch-read all targeted Report Buses in a single terminal invocation, e.g., `cat .apm/bus/<slug-1>/report.md .apm/bus/<slug-2>/report.md` (or platform equivalent). Continue to step 3.
...
3. Process report(s): for each Report Bus with content, process per `.claude/apm-guides/task-review.md` §3 Task Review Procedure.
