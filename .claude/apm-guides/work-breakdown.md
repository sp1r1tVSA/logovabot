...
- *`CLAUDE.md`:* Universal execution-level Rules applied during Task execution.
...
### 2.5 `CLAUDE.md` Standards

`CLAUDE.md` defines how work is performed - execution patterns that apply across all or most Tasks. Workers receive the entire file during Task execution, not just the APM Rules block. The block is a namespace to separate APM-managed standards from pre-existing content.
...
**Pre-existing content:** When `CLAUDE.md` already contains User standards, reference relevant pre-existing rules from the APM Rules block rather than duplicating. Do not modify pre-existing content unless the User explicitly requests it.

**Self-containedness:** Workers' working context is intentionally scoped to their Task Prompt and `CLAUDE.md` - the Spec, Plan, and external design artifacts are omitted by design. Standards referencing those documents undermine that scoping. Embed content directly.
...
Present analysis visibly in chat for the User to review per `.claude/skills/apm-communication/SKILL.md` §2.2 Visible Reasoning. At each approval gate, describe what was written, what comes next if approved, and ask for review.
...
1. Analyze design decisions from gathered context:
   ...
   - *Workspace.* From the workspace assessment during Context Gathering, document the project environment: directory structure, working repositories, reference repositories, authoritative document locations, existing `CLAUDE.md` content that was found.
...
Perform the following actions per §2.5 `CLAUDE.md` Standards:
1. Analyze for universal execution patterns across all planning sources:
   ...
   - **Classification:** Separate patterns that apply to all or most Tasks from narrowly Task-specific ones per §2.5 `CLAUDE.md` Standards. Most projects produce few genuinely universal rules - project-specific constraints and output specifications belong in the Spec or Task guidance even when they apply to multiple Workers.
   - **Existing standards:** what `CLAUDE.md` already contains; reference rather than duplicate.
2. Read `CLAUDE.md` (or confirm it does not exist), then write the APM_RULES block per §4.3 APM_RULES Block:
...
3. Pause for User review:
   ...
   - Ask User to review `CLAUDE.md` for accuracy.
   - If modifications needed, apply and repeat step 3.
   - If approved, state Work Breakdown is complete and all planning documents are created. Proceed to `.claude/commands/apm-1-initiate-planner.md` §4 Planning Phase Completion.
...
Below the frontmatter, the document starts with `# APM Spec` followed by two header sections: `## Overview` (3-5 sentences covering project type, core problem, essential scope, and success criteria) and `## Workspace` (project environment from the workspace assessment: directory structure, working repositories, reference repositories, authoritative document locations, and existing `CLAUDE.md` content). A single horizontal rule separates the header from the design decision content below. No horizontal rules within the content sections - `##` headings provide sufficient visual separation.
...
The namespace block structure for `CLAUDE.md`:
