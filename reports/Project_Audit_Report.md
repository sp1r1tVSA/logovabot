...
| **P3-3** | **Flip the defaults in the data layer.** Change `division_id=None, season_id=None` to required parameters on `get_standings` (2062), `get_teams_recent_form` (2695), `get_top_scorers` (4203), `get_all_unplayed_league_matches` (5214). The type checker and test suite will then enumerate every unscoped caller for you. **This is the single highest-value structural change in the plan** — it converts silent wrong-data bugs into loud failures. **Первый неявный вызыватель закрыт:** `Темшик таблица` (`handlers/text_commands.py`) звала `generate_league_table_image()` без аргументов, а генератор без `standings` уходил в `get_standings()` без дивизиона — в ветку по 16 именам КПЛ. Теперь команда резолвит дивизион (`handlers/base.py::resolve_division_id`), а генератор пробрасывает `division_id` в оба запроса; `tests/test_temshik_table_division_scope.py` это пинит. **Остаётся:** сами legacy-ветки `division_id is None` в `get_standings` / `get_teams_recent_form`, `get_all_clubs_summary` (5525) без параметра дивизиона вовсе, и собственно смена дефолтов на обязательные параметры. | `database.py` |
